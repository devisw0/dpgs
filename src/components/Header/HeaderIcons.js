...
const HeaderIcons = () => {
  ...
  let cartPopupCloseTimer = null;

  const handleCartPopupEnter = () => {
    if (cartPopupCloseTimer) {
      clearTimeout(cartPopupCloseTimer);
      cartPopupCloseTimer = null;
    }
    setCartPopupOpen(true);
  };

  const handleCartPopupLeave = () => {
    cartPopupCloseTimer = setTimeout(() => setCartPopupOpen(false), 180);
  };
  ...
  return (
    <Stack direction="row" alignItems="center" spacing={1}>
      ...
      <div
        ...
        onMouseEnter={handleCartPopupEnter}
        onMouseLeave={handleCartPopupLeave}
      >
        <IconButton color="inherit" size="large" onClick={() => navigate('/cart')}>
          <Badge badgeContent={cart.length} color="secondary" overlap="circular" anchorOrigin={{ vertical: 'top', horizontal: 'right' }}>
